...
public final class HudLayer extends Layer {
	...
	HudLayer(Context context) {
		...
		mTopRightButton.setSize((int) (100 * Gallery.PIXEL_DENSITY),
				(int) (94 * Gallery.PIXEL_DENSITY));

		mZoomInButton.setSize(43 * Gallery.PIXEL_DENSITY,
				43 * Gallery.PIXEL_DENSITY);
		mZoomOutButton.setSize(43 * Gallery.PIXEL_DENSITY,
				43 * Gallery.PIXEL_DENSITY);
		...
		PopupMenu.Option[] deleteOptions = {
				new PopupMenu.Option(context.getResources().getString(
						R.string.confirm_delete),
						resources.getDrawable(R.drawable.icon_delete),
						new Runnable() {
							public void run() {
								deleteSelection();
							}
		...
		MenuBar.Menu shareMenu = new MenuBar.Menu.Builder(context
				.getResources().getString(R.string.share))
				.icon(R.drawable.icon_share).onSelect(new Runnable() {
					public void run() {
						updateShareMenu();
					}
				}).build();

		MenuBar.Menu deleteMenu = new MenuBar.Menu.Builder(context
				.getResources().getString(R.string.delete))
				.icon(R.drawable.icon_delete).options(deleteOptions).build();

		MenuBar.Menu editMenu = new MenuBar.Menu.Builder(context.getResources()
				.getString(R.string.edit)).icon(R.drawable.icon_delete)
				.onSelect(new Runnable() {

					@Override
					public void run() {
						// TODO
						ArrayList<MediaBucket> selection = mGridLayer
								.getSelectedBuckets();
		...
		MenuBar.Menu moreMenu = new MenuBar.Menu.Builder(context.getResources()
				.getString(R.string.more)).icon(R.drawable.icon_more)
				.onSelect(new Runnable() {
					public void run() {
						buildMoreOptions();
					}
		...
		mNormalBottomMenu = new MenuBar.Menu[] { shareMenu, deleteMenu,
				editMenu, moreMenu };
		...
		mSelectionMenuTop.setMenus(new MenuBar.Menu[] {
				new MenuBar.Menu.Builder(context.getResources().getString(
						R.string.select_all)).onSelect(new Runnable() {
					public void run() {
				...
				new MenuBar.Menu.Builder(context.getResources().getString(
						R.string.deselect_all)).onSelect(new Runnable() {
					public void run() {
		...
		mFullscreenMenu.setMenus(new MenuBar.Menu[] {
				new MenuBar.Menu.Builder(context.getResources().getString(
						R.string.slideshow)).icon(R.drawable.icon_play)
				...
				new MenuBar.Menu.Builder(context.getResources().getString(
						R.string.menu)).icon(R.drawable.icon_more)
						.onSingleTapUp(new Runnable() {
							public void run() {
								if (getAlpha() == 1.0f)
	...
	private void buildMoreOptions() {
		...
		Option[] optionAll = new Option[] { new PopupMenu.Option(mContext
				.getResources().getString(R.string.details), mContext
				.getResources().getDrawable(R.drawable.ic_menu_view_details),
				new Runnable() {
					public void run() {
						ArrayList<MediaBucket> buckets = mGridLayer
								.getSelectedBuckets();
						// TODO
						...
						if (buckets == null) {
							...
							CharSequence[] strings = DetailMode
									.populateDetailModeStrings(mContext,
											buckets);
							if (strings == null) {
								foundDataToDisplay = false;
							} else {
						...
						if (foundDataToDisplay) {
							...
							((Gallery) mContext).getHandler().post(
									new Runnable() {
										public void run() {
											builder.show();
										}
		...
		Option[] optionSingle = new Option[] { new PopupMenu.Option(mContext
				.getResources().getString(R.string.show_on_map), mContext
				.getResources().getDrawable(R.drawable.ic_menu_mapmode),
				new Runnable() {
					public void run() {
						ArrayList<MediaBucket> buckets = mGridLayer
								.getSelectedBuckets();
						MediaItem item = MediaBucketList
								.getFirstItemSelection(buckets);
						if (item == null) {
							return;
						}
						mGridLayer.deselectAll();
		...
		Option[] optionImageMultiple = new Option[] {
				new PopupMenu.Option(mContext.getResources().getString(
						R.string.rotate_left), mContext.getResources()
						.getDrawable(R.drawable.ic_menu_rotate_left),
						new Runnable() {
							public void run() {
				...
				new PopupMenu.Option(mContext.getResources().getString(
						R.string.rotate_right), mContext.getResources()
						.getDrawable(R.drawable.ic_menu_rotate_right),
						new Runnable() {
							public void run() {
								mGridLayer.rotateSelectedItems(90.0f);
							}
		...
		if (isPicasa) {
			optionImageSingle = new Option[] { new PopupMenu.Option(mContext
					.getResources().getString(R.string.set_as_wallpaper),
					mContext.getResources().getDrawable(
						...
						public void run() {
							...
							if (item.mParentMediaSet.mPicasaAlbumId != Shared.INVALID) {
								...
								((Gallery) mContext).startActivityForResult(
										intent, 0);
							}
						}
					}) };
			...
			optionImageSingle = new Option[] {
					new PopupMenu.Option((isPicasa) ? mContext.getResources()
							.getString(R.string.set_as_wallpaper) : mContext
								...
								public void run() {
									...
									if (item.mParentMediaSet.mPicasaAlbumId != Shared.INVALID) {
										...
										intent.setClassName(
												...
												"com.cooliris.media.Photographs");
										intent.setData(Uri
												.parse(item.mContentUri));
										intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
										((Gallery) mContext)
												.startActivityForResult(intent,
														0);
									} else {
										Intent intent = Util.createSetAsIntent(
												Uri.parse(item.mContentUri),
												item.mMimeType);
										intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
										((Gallery) mContext).startActivity(Intent
												.createChooser(
														intent,
														((Gallery) mContext)
					...
					new PopupMenu.Option(mContext.getResources().getString(
							R.string.crop), mContext.getResources()
							.getDrawable(R.drawable.ic_menu_crop),
							new Runnable() {
								public void run() {
									ArrayList<MediaBucket> buckets = mGridLayer
											.getSelectedBuckets();
	...
	public void updateNumItemsSelected(int numItems) {
		String items = " "
				+ ((numItems == 1) ? mContext.getString(R.string.item)
						: mContext.getString(R.string.items));
		Menu menu = new MenuBar.Menu.Builder(numItems + items).config(
				MenuBar.MENU_TITLE_STYLE_TEXT).build();
	...
	protected void onSizeChanged() {
		...
		mTimeBar.setPosition(0f, height - TimeBar.HEIGHT
				* Gallery.PIXEL_DENSITY);
		mTimeBar.setSize(width, TimeBar.HEIGHT * Gallery.PIXEL_DENSITY);
		mSelectionMenuTop.setPosition(0f, 0);
		mSelectionMenuTop
				.setSize(width, MenuBar.HEIGHT * Gallery.PIXEL_DENSITY);
		mSelectionMenuBottom.setPosition(0f, height - MenuBar.HEIGHT
				* Gallery.PIXEL_DENSITY);
		mSelectionMenuBottom.setSize(width, MenuBar.HEIGHT
				* Gallery.PIXEL_DENSITY);

		mFullscreenMenu.setPosition(0f, height - MenuBar.HEIGHT
				* Gallery.PIXEL_DENSITY);
		...
		mZoomOutButton.setPosition(width - mZoomInButton.getWidth(),
				mZoomInButton.getHeight());
	}
	...
	private void computeSizeForPathbar() {
		...
		mPathBar.setSize(pathBarWidth,
				FloatMath.ceil(39 * Gallery.PIXEL_DENSITY));
	...
	private void updateViews() {
		...
		final boolean stackMode = state == GridLayer.STATE_MEDIA_SETS
				|| state == GridLayer.STATE_TIMELINE;
	...
	public boolean update(RenderView view, float frameInterval) {
		...
		mAnimAlpha = FloatUtils.animate(mAnimAlpha, mAlpha, frameInterval
				* factor);
		...
		if (mAnimAlpha != mAlpha
				|| (mTimeElapsedSinceFullOpacity < 5.0f && !timeElapsedSinceFullOpacity_Reset))
			return true;
	...
	private void updateShareMenu() {
		...
		if (uris.size() != 0) {
			...
			for (int i = 0; i != numActivities; ++i) {
				...
				options[i] = new PopupMenu.Option(label,
						info.loadIcon(packageManager), new Runnable() {
							public void run() {
	...
	public void swapFullscreenLabel() {
		mCachedCurrentLabel = (mCachedCurrentLabel == mCachedCaption || mCachedCaption == null) ? mCachedPosition
				: mCachedCaption;
